...
public class InvalidCustomOptionListException extends ValidationException {
  ...
  public enum InvalidReasonEnum {
    OPTION_LIST_EMPTY("Question option list is empty. There should be at least %s options."),
    OPTION_LIST_TEXT_EMPTY("Question option list text is empty."),

    OPTION_LIST_TOO_SHORT("Question option list too small. There should be at least %s options."),
    OPTION_LIST_TOO_BIG("Question option list too big. There should be at max %s options."),
    INVALID_OPTIONS_IDS("Question option list must have ids from 1 to last with no missing values and without " +
        "duplicated " +
        "ids. The ids in the sorted order: %s");
