...
public class EmployeeAssessmentRepositoryV2 extends AbstractCrudRepositoryV2<EmployeeAssessmentV2> {

    @Inject
    public EmployeeAssessmentRepositoryV2(DynamoDbEnhancedClient dynamoDbEnhancedClient) {
        super(dynamoDbEnhancedClient, EmployeeAssessmentV2.class, "EmployeeAssessment");
    }

    @VisibleForTesting
    public EmployeeAssessmentRepositoryV2(DynamoDbEnhancedClient dynamoDbEnhancedClient, String tableName) {
        super(dynamoDbEnhancedClient, EmployeeAssessmentV2.class, tableName);
    }
    ...
    public boolean existsByAssessmentMatrixAndEmployeeEmail(String assessmentMatrixId, String employeeEmail) {
        ...
        try {
            DynamoDbIndex<EmployeeAssessmentV2> gsi = getTable().index("assessmentMatrixId-employeeEmail-index");
            ...
            List<EmployeeAssessmentV2> results = gsi.query(queryRequest)
                .stream()
     ...
     */
    public List<EmployeeAssessmentV2> findByAssessmentMatrixId(String assessmentMatrixId, String tenantId) {
        try {
            DynamoDbIndex<EmployeeAssessmentV2> gsi = getTable().index("assessmentMatrixId-employeeEmail-index");
