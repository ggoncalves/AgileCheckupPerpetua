...
public class QuestionRepositoryV2 extends AbstractCrudRepositoryV2<QuestionV2> {

    @Inject
    public QuestionRepositoryV2(DynamoDbEnhancedClient dynamoDbEnhancedClient) {
        super(dynamoDbEnhancedClient, QuestionV2.class, "Question");
    }

    @VisibleForTesting
    public QuestionRepositoryV2(DynamoDbEnhancedClient dynamoDbEnhancedClient, String tableName) {
        super(dynamoDbEnhancedClient, QuestionV2.class, tableName);
    }
    ...
    public boolean existsByCategoryId(String matrixId, String categoryId, String tenantId) {
        ...
        List<QuestionV2> results = getTable().scan(scanRequest)
                .stream()
                .flatMap(page -> page.items().stream())
                .limit(1)
                .collect(Collectors.toList());
