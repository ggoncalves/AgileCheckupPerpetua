...
public class QuestionTableRunner implements CrudRunner {
  ...
  private void testCreateOperations() {
    ...
    if (customQuestionOpt.isPresent()) {
      ...
      log.info("✓ Created CUSTOMIZED question: {} (ID: {}) with {} options", customQuestion.getQuestion(), customQuestion.getId(), customOptions.size());
    }
    else {
  ...
  private void cleanupAllTestData() {
    ...
    log.info("Cleanup completed. Cleaned {} questions, 1 matrix, 1 cycle, 1 company", createdQuestions.size());
  }
