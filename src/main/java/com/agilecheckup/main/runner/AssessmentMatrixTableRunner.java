...
public class AssessmentMatrixTableRunner implements CrudRunner {
  ...
  private void testQueryOperations() {
    ...
    if (dashboardOpt.isPresent()) {
      ...
      log.info("✓ Retrieved assessment dashboard - Matrix: {}, Total employees: {}", dashboard.getMatrixName(), dashboard.getTotalEmployees());
    }
    else {
  ...
  private void cleanupAllTestData() {
    ...
    log.info("Cleanup completed. Cleaned {} matrices, {} questions, 1 cycle, 1 company", createdMatrices.size(), createdQuestions.size());
  }
