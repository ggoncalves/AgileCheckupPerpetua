...
public class AnswerTableRunner implements CrudRunner {
  ...
  private void setupTestDependencies() {
    ...
    if (assessmentOpt.isPresent()) {
      ...
      log.info("Created test employee assessment: {} (ID: {})", testEmployee.getName(), testEmployeeAssessment.getId());
    }
    else {
  ...
  private void createTestQuestions() {
    ...
    String[] questionTexts = {"Is your team following agile principles?", "How satisfied are you with team communication?", "Rate your team's productivity level", "What improvements would you suggest?", "Which tools does your team use most?"
    };

    QuestionType[] questionTypes = {QuestionType.YES_NO, QuestionType.STAR_FIVE, QuestionType.ONE_TO_TEN, QuestionType.OPEN_ANSWER, QuestionType.CUSTOMIZED
    };
  ...
  private void testCreateOperations() {
    ...
    if (yesNoQuestion != null) {
      ...
      if (yesNoAnswerOpt.isPresent()) {
        ...
        log.info("✓ Created YES_NO answer: {} -> {} (Score: {})", yesNoQuestion.getQuestion(), yesNoAnswer.getValue(), yesNoAnswer.getScore());
      }
      else {
    ...
    if (starFiveQuestion != null) {
      ...
      if (starFiveAnswerOpt.isPresent()) {
        ...
        log.info("✓ Created STAR_FIVE answer: {} -> {} stars (Score: {})", starFiveQuestion.getQuestion(), starFiveAnswer.getValue(), starFiveAnswer.getScore());
      }
      else {
    ...
    if (oneToTenQuestion != null) {
      ...
      if (oneToTenAnswerOpt.isPresent()) {
        ...
        log.info("✓ Created ONE_TO_TEN answer: {} -> {} (Score: {})", oneToTenQuestion.getQuestion(), oneToTenAnswer.getValue(), oneToTenAnswer.getScore());
      }
      else {
    ...
    if (customQuestion != null) {
      ...
      if (customAnswerOpt.isPresent()) {
        ...
        log.info("✓ Created CUSTOMIZED answer: {} -> Option {} (Score: {})", customQuestion.getQuestion(), customAnswer.getValue(), customAnswer.getScore());
      }
      else {
  ...
  private void testUpdateOperations() {
    ...
    if (answerToUpdate != null) {
      ...
      if (updatedAnswerOpt.isPresent()) {
        ...
        log.info("✓ Updated answer: {} -> {} (Score: {} -> {})", originalValue, updatedAnswer.getValue(), originalScore, updatedAnswer.getScore());
  ...
  private void testAssessmentIntegration() {
    ...
    if (currentAssessmentOpt.isPresent()) {
      ...
      if (currentAssessment.getAnsweredQuestionCount() >= expectedCount) {
        ...
        log.warn("  Answered question count mismatch: expected at least {}, got {}", expectedCount, currentAssessment.getAnsweredQuestionCount());
      }
    }
  ...
  private void cleanupAllTestData() {
    ...
    log.info("Cleanup completed. Cleaned {} answers, {} questions, 1 assessment, 1 matrix, 1 cycle, 1 team, 1 department, 1 company", createdAnswers.size(), createdQuestions.size());
  }
