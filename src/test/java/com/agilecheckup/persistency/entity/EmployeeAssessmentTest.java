...
class EmployeeAssessmentTest {
  ...
  void testEmployeeAssessmentConstruction() {
    NaturalPerson employee = NaturalPerson.builder().id("person-123").name("John Doe").email("john.doe@example.com").documentNumber("123456789").personDocumentType(PersonDocumentType.CPF).gender(Gender.MALE).genderPronoun(GenderPronoun.HE).build();

    EmployeeAssessmentScore score = EmployeeAssessmentScore.builder().score(85.5).build();

    Date lastActivity = new Date();

    EmployeeAssessment assessment = EmployeeAssessment.builder().id("assessment-123").assessmentMatrixId("matrix-123").employee(employee).teamId("team-123").employeeAssessmentScore(score).assessmentStatus(AssessmentStatus.IN_PROGRESS).answeredQuestionCount(5).employeeEmailNormalized("john.doe@example.com").lastActivityDate(lastActivity).build();
  ...
  void testSetEmployeeUpdatesNormalizedEmail() {
    NaturalPerson initialEmployee = NaturalPerson.builder().id("person-initial").name("Initial Name").email("initial@example.com").build();

    EmployeeAssessment assessment = EmployeeAssessment.builder().id("assessment-123").assessmentMatrixId("matrix-123").teamId("team-123").employee(initialEmployee).build();
    assessment.setTenantId("tenant-123");

    NaturalPerson employee = NaturalPerson.builder().id("person-123").name("Jane Smith").email("JANE.SMITH@EXAMPLE.COM").build();
  ...
  void testSetEmployeeWithBlankEmailSetsNormalizedEmailToNull() {
    ...
    NaturalPerson employee = NaturalPerson.builder().id("person-123").name("Jane Smith").email("   ").build();
  ...
  void testDefaultAnsweredQuestionCountIsZero() {
    NaturalPerson employee = NaturalPerson.builder().id("person-123").name("Test Employee").email("test@example.com").build();

    EmployeeAssessment assessment = EmployeeAssessment.builder().id("assessment-123").tenantId("tenant-123").assessmentMatrixId("matrix-123").teamId("team-123").employee(employee).build();
