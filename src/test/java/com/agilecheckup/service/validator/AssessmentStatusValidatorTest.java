...
class AssessmentStatusValidatorTest {
  ...
  @ParameterizedTest
  @CsvSource({"INVITED, CONFIRMED", "INVITED, IN_PROGRESS", "CONFIRMED, IN_PROGRESS", "IN_PROGRESS, COMPLETED"
  })
  ...
  void shouldThrowExceptionForNullCurrentStatus() {
    assertThatThrownBy(() -> AssessmentStatusValidator.validateTransition(null, AssessmentStatus.INVITED)).isInstanceOf(NullPointerException.class);
  }

  @Test
  void shouldThrowExceptionForNullNewStatus() {
    assertThatThrownBy(() -> AssessmentStatusValidator.validateTransition(AssessmentStatus.INVITED, null)).isInstanceOf(NullPointerException.class);
  }

  @Test
  void shouldThrowExceptionForNullStatusName() {
    assertThatThrownBy(() -> AssessmentStatusValidator.validateStatus(null)).isInstanceOf(NullPointerException.class);
  }
}